    ...
    function apply(ctx) {
      ...
      ctx.slots.inject('conversation.view', () => {
        ...
        return ctx.slots.register({
          ...
          locale: NS,
          label: () => t('view.tidy'),
          // `conversation.view` is not a session-scope seat, so dsh 0.1.5 no
          // longer places `sessionId` in the render props (0.1.1-rc.2 did).
          // The framework hands it to this callback instead — the same shape
          // the official `ui-trajectory` registrant uses. Returning it merges
          // `sessionId` back into the props, which `sessionIdOf` already reads,
          // so the older dsh line keeps working unchanged.
