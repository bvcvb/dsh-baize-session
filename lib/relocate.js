...
import { listMessages, messageIdOf, readMessageEvent, renderInjection, sessionDisplayName, } from "./core.js";
import { getPersistence, listStored, readStored } from "./persistence.js";
/**
...
export function assertAbsolutePath(path) {
    if (path.length === 0)
        throw new RelocationError('目标路径不能为空。');
    const absolute = path.startsWith('/') || /^[A-Za-z]:[\\/]/.test(path);
    if (!absolute)
        throw new RelocationError(`目标路径必须是绝对路径：${path}`);
}
/** Create the shared relocation runtime for one plugin instance. */
export function createRelocation(ctx, options) {
    ...
    const session = (sessionId) => ctx.sessions.get(sessionId);
    const basketOf = (sessionId) => {
     ...
     */
    const inspectionOf = async (sessionId, signal) => {
        const live = session(sessionId);
        if (live !== undefined)
            return { events: liveEventsOf(live), cwd: live.header?.cwd ?? '', readable: true };
        const store = persistence();
        if (store === undefined)
            return { events: [], cwd: '', readable: false };
     ...
     */
    const isBlank = (events, readable) => {
        if (!readable)
            return false;
        return !events.some(event => typeof event === 'object' && event !== null && event.type === 'turn/start');
    };
    ...
    const messagesOf = async (sessionId, signal) => listMessages((await inspectionOf(sessionId, signal)).events);
    /**
    ...
    const take = async (sessionId, refs) => {
        ...
        if (wantedSeqs.size === 0 && wantedIds.size === 0)
            throw new RelocationError('请给出要收集的消息。');
        ...
        for (const seq of wanted) {
            ...
            if (basket.some(item => item.sessionId === sourceId && item.seq === seq))
                continue;
            basket.push({
                sessionId: sourceId,
                cwd,
    ...
    const untake = (sessionId, ref) => {
        const basket = basketOf(sessionId);
        const before = basket.length;
        const kept = basket.filter((item) => {
            if (ref.sourceId !== undefined && item.sessionId !== ref.sourceId)
                return true;
            if (ref.messageId !== undefined && item.messageId === ref.messageId)
                return false;
            if (ref.seq !== undefined && item.seq === ref.seq)
                return false;
            return true;
        });
        basket.length = 0;
    ...
    const drop = (sessionId) => { basketOf(sessionId).length = 0; };
    /** Build the message carrying the collected history. */
    const buildMessage = (sessionId, targetCwd) => {
        const basket = basketOf(sessionId);
        if (basket.length === 0)
            throw new RelocationError('还没有选任何消息。');
    ...
    const targetCwdOf = (target) => target.kind === 'new' ? target.cwd : (session(target.sessionId)?.header?.cwd ?? '');
    /**
     ...
     * here" case.
     */
    const projectRefs = (all, currentCwd) => {
        const real = (path) => all.filter(ref => ref.cwd === path && !ref.blank && !ref.archived).length;
        const rows = [];
        const seen = new Set();
        try {
            const registry = workspaceRegistry();
        ...
        for (const ref of all) {
            if (ref.cwd.length === 0 || seen.has(ref.cwd) || ref.blank || ref.archived)
                continue;
        ...
        if (rows.length === 0) {
            ...
            for (const [path, count] of paths)
                rows.push({ path, sessions: count, isCurrent: path === currentCwd });
        }
        return rows.sort((a, b) => Number(b.isCurrent) - Number(a.isCurrent));
    };
    const panelState = async (sessionId, signal) => {
        const live = session(sessionId);
    ...
    const estimate = (sessionId, target) => {
        if (basketOf(sessionId).length === 0)
            return 0;
        return ctx.tokenMeter.estimateMessage(buildMessage(sessionId, targetCwdOf(target)));
    };
    const relocate = async (sessionId, target) => {
        ...
        if (target.kind === 'existing') {
            ...
            if (tokens > options.maxInjectTokens) {
                throw new RelocationError(`选中内容约 ${tokens} tokens，超过上限 ${options.maxInjectTokens}。请少选几条，或调大 maxInjectTokens。`);
            }
        ...
        if (registry === undefined) {
            ...
            try {
                workspace = (await registry.resolveByPath(targetCwd)
                    ?? await registry.create(targetCwd));
                if (workspace === undefined)
                    console.warn('[baize-session] no workspace entity for ' + targetCwd);
            }
            catch (e) {
        ...
        const created = ctx.sessions.create(`session-${crypto.randomUUID()}`, {
            seed: [seedEvent],
        ...
        if (workspace?.attachSession !== undefined) {
            await workspace.attachSession(created.id).catch((e) => {
