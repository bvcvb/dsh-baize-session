...
function targetOf(body: Record<string, unknown>): RelocateTarget {
  const raw = body.target
  if (typeof raw !== 'object' || raw === null) throw new RelocationError('缺少 target。')
  const target = raw as Record<string, unknown>
  if (target.kind === 'existing') {
    const sessionId = String(target.sessionId ?? '')
    if (sessionId.length === 0) throw new RelocationError('target.sessionId 不能为空。')
  ...
  return ctx.webServer.register({
    ...
    handler: async (req: IncomingMessage, res: ServerResponse) => {
      ...
      if (method === 'POST') {
        ...
        if (body === undefined) { sendJson(res, 400, { error: 'invalid JSON body' }); return }
