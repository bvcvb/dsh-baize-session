...
function contentToText(content: unknown): string {
  ...
  for (const block of content) {
    ...
    if (record.type === 'text' && typeof record.text === 'string') { parts.push(record.text); continue }
 ...
 */
function messagePayload(event: unknown): {
  content: unknown
  id?: string
  ...
  const inner = (typeof nested === 'object' && nested !== null ? nested : record) as Record<string, unknown>
 ...
 */
export function readMessageEvent(event: unknown): { role: MessageRole; text: string } | null {
  if (typeof event !== 'object' || event === null) return null
 ...
 */
function roleOfEvent(type: unknown, source: Record<string, unknown> | undefined): MessageRole {
 ...
 */
export function messageIdOf(event: unknown): string | undefined {
...
export function listMessages(events: readonly unknown[]): MessageRef[] {
...
function groupBySource(items: readonly CollectedItem[]): { sessionId: string; cwd: string; items: CollectedItem[] }[] {
  const groups = new Map<string, { sessionId: string; cwd: string; items: CollectedItem[] }>()
  for (const item of items) {
    const group = groups.get(item.sessionId)
    if (group === undefined) groups.set(item.sessionId, { sessionId: item.sessionId, cwd: item.cwd, items: [item] })
 ...
 */
export function renderInjection(items: readonly CollectedItem[], targetCwd: string): string {
  const blocks: string[] = [INJECTION_HEADER, '']
  for (const group of groupBySource(items)) {
    blocks.push(`【来自会话 ${group.sessionId.slice(0, 8)} / 工作区 ${group.cwd || '(未知)'}】`)
    for (const item of group.items) blocks.push(`${item.role}: ${item.text}`)
    blocks.push('')
  }
  blocks.push('---')
...
export function totalChars(items: readonly CollectedItem[]): number {
  return items.reduce((n, item) => n + item.text.length, 0)
}

/** Compact listing of the basket, one line per item. */
export function formatBasket(items: readonly CollectedItem[]): string {
  if (items.length === 0) return '篮子为空。'
 ...
 */
export function loggedSessionTitle(events: readonly unknown[]): string | undefined {
 ...
 */
export function sessionDisplayName(events: readonly unknown[]): string {
