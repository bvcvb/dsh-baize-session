...
export interface PersistenceLike {
  ...
  inspect?(
    id: string,
    signal?: AbortSignal,
  ): Promise<{ readonly meta?: { readonly cwd?: string }; readonly events?: readonly unknown[] } | undefined>
  ...
  open?(id: string, access: 'read' | 'write', options?: { readonly signal?: AbortSignal }): Promise<HandleLike | undefined>
}
 ...
 */
export const getPersistence = (ctx: unknown): PersistenceLike | undefined => {
  const get = (ctx as { get?(name: string): unknown } | undefined)?.get
  return typeof get === 'function' ? (get('sessionPersistence') as PersistenceLike | undefined) : undefined
}

/** Which read stack this backend speaks. */
export const readStackOf = (store: PersistenceLike | undefined): ReadStack => {
  if (store === undefined) return 'none'
  if (typeof store.inspect === 'function') return 'inspect'
  if (typeof store.open === 'function') return 'handle'
  return 'none'
}
...
export const listRaw = async (
  store: PersistenceLike | undefined,
  signal?: AbortSignal,
 ...
 */
const rowOf = (raw: unknown): SessionRow | undefined => {
