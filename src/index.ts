...
function invocationSessionId(invocation: CommandInvocation): string | undefined {
  const session = (invocation.agent as { session?: { id?: string } } | undefined)?.session
  const id = session?.id
...
async function handle(
  ...
  if (sessionId === undefined) return { kind: 'error', text: '当前没有会话，无法执行。' }
  ...
  try {
    switch (verb) {
      ...
      case 'status': {
        ...
        for (const message of recent) lines.push(`  [${message.seq}] ${message.role}: ${message.preview}`)
        if (recent.length === 0) lines.push('  （本会话还没有可收集的消息）')
      ...
      case 'take': {
        ...
        if (missing.length > 0) lines.push(`未找到或不可收集：${missing.join('、')}`)
      ...
      case 'list': {
        ...
        for (const ref of all) {
          ...
          if (bucket === undefined) byProject.set(ref.cwd || '(未知)', [ref])
        ...
        if (hidden > 0) lines.push('', `（另有 ${hidden} 个空会话/已归档会话未列出）`)
        return { kind: 'success', text: lines.join('\n') }
      }
      ...
      case 'move': {
        const cwd = args[0]
        if (cwd === undefined) return { kind: 'error', text: `请给出目标工作区的绝对路径。\n${USAGE}` }
      ...
      case 'append': {
        const id = args[0]
        if (id === undefined) return { kind: 'error', text: `请给出目标会话 id（用 list 查看）。\n${USAGE}` }
    ...
    if (e instanceof RelocationError) return { kind: 'error', text: e.message }
    throw e
  }
}
